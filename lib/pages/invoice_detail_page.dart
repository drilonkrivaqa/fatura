      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            Text('Date: ${invoice.date.toLocal().toString().split(' ').first}'),
            Text('Due: ${invoice.dueDate.toLocal().toString().split(' ').first}'),
            ...
            ...invoice.items.map((item) => ListTile(
                  ...
                  subtitle: Text(
                      '${item.quantity.toStringAsFixed(2)} ${item.unit} x ${item.unitPrice.toStringAsFixed(2)} (discount ${item.discount.toStringAsFixed(1)}%)'),
